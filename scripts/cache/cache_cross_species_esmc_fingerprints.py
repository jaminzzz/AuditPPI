#!/usr/bin/env python3
"""Cache ESM-C SAE-layer representations + SAE fingerprints for cross-species PPI.

Mirrors cache_esmc_fingerprints.py but scans the cross-species CSVs: human
train/test plus held-out ecoli/fly/mouse/worm/yeast tests. Each unique sequence
is encoded once and written to CROSS_SPECIES_SEQ_CACHE (consumed by the audit
via conf.paths). Default --layer is ESMC_SAE_DEFAULT_LAYER (60); non-default
layers write a layer-tagged path so they do not clobber the default cache.
...
Run with the E1 conda env (Biohub transformers fork). CSVs are read-only.
  ...
  /data/wmzhu/anaconda3/envs/E1/bin/python \
      scripts/cache/cache_cross_species_esmc_fingerprints.py --layer 80
"""
...
from conf.paths import (
    ESMC_MODEL, ESMC_SAE, CROSS_SPECIES_DIR, CROSS_SPECIES_SEQ_CACHE,
)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Cache cross-species ESM-C pooled reps + SAE features")
    ...
    p.add_argument("--data-dir", type=Path, default=CROSS_SPECIES_DIR)
    ...
    p.add_argument(
        ...
        help=f"output cache path (default: {CROSS_SPECIES_SEQ_CACHE.name}, "
...
def main() -> None:
    ...
    if args.out is None:
        args.out = layer_tagged_cache_path(CROSS_SPECIES_SEQ_CACHE, args.layer)
    ...
    for name in CSV_FILES:
        ...
        df = pd.read_csv(path, usecols=["query", "text"])
    ...
    print(f"[data] unique sequences: {len(seqs)} (limit={args.limit or 'none'})", flush=True)
    ...
    cache = {
        ...
        "meta": {
            ...
            "data_dir": str(args.data_dir), "csv_files": CSV_FILES,
        },
    }
