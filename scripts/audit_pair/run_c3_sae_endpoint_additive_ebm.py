...
def main() -> None:
    ...
    train = load_split(args.rep, "train")
    val = load_split(args.rep, "val")
    test = load_split(args.rep, "test")
    print(
    ...
    (args.out_dir / f"{stem}_metrics.json").write_text(json.dumps(result, indent=2))
    ...
    print("[done] wrote", args.out_dir / f"{stem}_metrics.json", flush=True)
    print(
